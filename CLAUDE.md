...
本仓库是一个 **skill 分发包**，不是可运行的应用。它的产物是一份 Markdown 驱动的 Agent 技能 `ruifeng-data-governance`（睿锋智链汽车配件数据治理），通过 `npm install` 安装到本地两个位置供 Claude Code / Hermes 加载：

- `~/.claude/skills/ruifeng-data-governance/`
- `~/.hermes/skills/ruifeng-data-governance/`
...
> 注意：磁盘目录名为 `ruifeng-data-cleaning`，但包名/技能名已更名为 `ruifeng-data-governance`（v3.0.0 重构）。两个名字在文档里都会出现。
...
用户 / Hermes Agent
   │  加载技能
   ▼
ruifeng-data-governance skill（本仓库）   ──引用──►   cli-anything-platform-service CLI（独立 Python 仓库）
