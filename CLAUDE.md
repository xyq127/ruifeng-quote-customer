...
本仓库是一个 **skill 分发包**，不是可运行的应用。它的产物是一份 Markdown 驱动的 Agent 技能 `ruifeng-quote-customer`（睿锋-客户询价），通过 `npm install` 安装到本地两个位置供 Claude Code / Hermes 加载：

- `~/.claude/skills/ruifeng-quote-customer/`
- `~/.hermes/skills/ruifeng-quote-customer/`
...
> 注意：包名/磁盘目录名为 `ruifeng-quote-customer`，技能显示名为「睿锋-客户询价」。早期文档或历史 commit 中可能仍出现旧名 `ruifeng-data-governance-customer` / 「睿锋数据治理」。
...
用户 / Hermes Agent
   │  加载技能
   ▼
ruifeng-quote-customer skill（本仓库）   ──引用──►   cli-anything-platform-service CLI（独立 Python 仓库）
