...
@click.option("--keyword", required=True, help="搜索关键词 (8位数字/OE号/DAC编号/code)")
...
def backend_search(keyword, query_type, page, size, with_details, use_json):
    """查询睿锋后台产品库.
    ...
    result = backend.get("/api/principal/product/list", params=params)
...
@click.option("--json", "use_json", is_flag=True, help="JSON 输出")
def backend_detail(product_id, use_json):
    """获取产品关联编号和参数详情."""
    backend = get_backend()
    results = {}
