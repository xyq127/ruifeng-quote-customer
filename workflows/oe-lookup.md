...
输入是图片（`.jpg/.jpeg/.png/.webp/.bmp` 路径或图片 URL）时，先读出编号，**不可跳过直接查询**。客户版**零配置**：Agent 直接用 Read 工具打开图片，用自身视觉读出编号——无需 SiliconFlow Key、无需调用识别脚本。图片 URL 先下载到本地再 Read。

**处理：**
1. 读出编号文本，**复述给用户确认**（视觉易误读 0/O、8/B、5/S，错号会污染整链）。
...
**失败处理：**
- 读图结果全模糊 / 无可用编号 → 请用户提供更清晰图片或直接给文本编号，**不强行查询**。
...
### 校验结论（内部用，不输出给客户）
泰安联与17vin一致，但后台未找到对应产品。→ 置信度 B-待补充（仅内部记录，不在客户回复中展示）。
